...
map.on("locationfound", function(location) {
  var userMark = L.userMarker(location.latlng, {
    smallIcon: true,
    circleOpts: {
      weight: 1,
      opacity: 0.3,
...
function pieChart(cluster) {
  ...
  var data = _.chain(children)
    .countBy(function(i) {
      return i.options.icon.options.color
    })
...
function loadIncidenLayerXHR(in_relink, in_lyr_type, in_ref_lyr) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $("#" + in_lyr_type + "Checkbox").change(function() {
        this.checked ? incidentData.addLayers(in_ref_lyr) : incidentData.removeLayers(in_ref_lyr);
      });
      refLayers.push({
        id: in_lyr_type,
        lyr: in_ref_lyr
